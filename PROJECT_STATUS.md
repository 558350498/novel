...
建立一套服务于中文叙事机制与文风迁移的文本分析与生成辅助流程：参考《安达与岛村》第 5 卷「岛村之刃」的关系机制、情绪张力结构和可迁移文风表面，但不复制原文、不长篇摘抄、不照搬句式或情节结构。最终产出可用的写作 prompt、可迁移规则、审稿清单，以及轻量文本风格评估工具。
...
- 如果后续加入日文原版，它作为上游参考文本校准机制；中文译文和本项目 prompt 作为本地适配层，服务于中文叙事机制与文风迁移。
...
- 最终创作目标：中文候选文本，不回推日文原文，不复制原文表达；但允许并需要贴近可迁移的节奏、语气、意识流形状和接收端错位机制。
...
- 能清晰解释「岛村之刃」的情绪张力机制。
- 能把分析转成可执行写作规则。
- prompt 能引导生成中文候选文本，贴近源切片的可迁移文风机制，而不是复制原文或照搬情节。
